...
from .forms import Step1Form, Step2Form, IndividualForm


# Helper function to get form based on step
def get_form(step):
    forms = {
        1: Step1Form,
        2: Step2Form,
...
# Multi-step startup registration view
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if "company_logo" in form_data and form_data["company_logo"]:
                file = form_data.pop("company_logo")
                file_path = f'logos/{file.name}'
            ...
            if "business_certificate" in request.FILES:
                ...
                with open(f'media/{file_path}', 'wb+') as destination:
                    for chunk in file.chunks():
                        destination.write(chunk)
                form_data["business_certificate"] = file_path
...
# Complete registration view — save data to DB
...
def individual_reg(request):
    if request.method == 'POST':
        ...
        form = IndividualForm()
    return render(request, 'register/individualreg.html', {'form': form})
...
def dashboard(request):
    ...
    common_industry_data = Registration.objects.values('sector').annotate(count=Count('sector')).order_by('-count').first()
    common_industry = common_industry_data['sector'] if common_industry_data else 'N/A'
